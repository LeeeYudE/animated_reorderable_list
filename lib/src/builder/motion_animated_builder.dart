...
class MotionBuilder<E> extends StatefulWidget {
  ...
  final void Function(int index)? onReorderLongTap;
  ...
  static MotionBuilderState of(BuildContext context) {
    final MotionBuilderState? result = context.findAncestorStateOfType<MotionBuilderState>();
    assert(() {
      if (result == null) {
        throw FlutterError(
...
class MotionBuilderState extends State<MotionBuilder> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  final List<_ActiveItem> _incomingItems = <_ActiveItem>[];
  final List<_ActiveItem> _outgoingItems = <_ActiveItem>[];
  int _itemsCount = 0;

  Map<int, MotionData> childrenMap = <int, MotionData>{};
  final Map<int, MotionAnimatedContentState> _items = <int, MotionAnimatedContentState>{};
  ...
  Drag? _dragStart(Offset position) {
    ...
    if (widget.onReorderLongTap != null) {
      disableDrag = true;
      _onLongPressTimer = Timer(const Duration(milliseconds: 1000), () {
        disableDrag = false;
        widget.onReorderLongTap?.call(_dragIndex!);
      });
    }
    item.rebuild();
  ...
  void _dropCompleted() {
    ...
    final int fromIndex = _dragIndex ?? 0;
    final int toIndex = _insertIndex ?? 0;
  ...
  Offset calculateNextDragOffset(int index) {
    ...
    if (isGrid) {
      ...
      final Offset offset = _extentOffset(_dragInfo!.itemExtent, scrollDirection);
  ...
  void insertItem(int index, {required Duration insertDuration}) {
    ...
    final motionData = MotionData(endOffset: Offset.zero, startOffset: Offset.zero);
  ...
  Widget build(BuildContext context) {
    ...
    return widget.delegateBuilder != null
        ? SliverGrid(gridDelegate: widget.delegateBuilder!, delegate: _createDelegate())
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    return MotionAnimatedContent(
      ...
      updateMotionData: (MotionData motionData) {
        ...
        childrenMap[index] = motionData.copyWith(startOffset: itemOffset, endOffset: itemOffset, visible: true);
      },
    ...
    if (widget.buildDefaultDragHandles) {
      switch (Theme.of(context).platform) {
        ...
        case TargetPlatform.iOS:
          return ReorderableGridDelayedDragStartListener(key: itemGlobalKey, index: index, child: item);
      }
    }
  ...
  Widget _insertItemBuilder(_ActiveItem? incomingItem, Widget child) {
    ...
    return SizeTransition(sizeFactor: sizeAnimation, child: widget.insertAnimationBuilder(context, child, animation));
  }
}
